...
const Dashboard = () => {
  ...
  // 1. Check Auth & Load Cart/Orders on Mount
  useEffect(() => {
  ...
  // Fix: Prioritize product_id if it exists (for Cart items), otherwise use id (for Shop items)
  const addToCart = async (product) => {
    if (!user) return;
    try {
      const productData = {
        // CHANGED THIS LINE: Check product_id first!
          ...
          <Card className="border-none shadow-lg">
            <CardHeader>
              <CardTitle className="flex items-center gap-2">
                ...
                Your Cart {loading && <span className="text-sm font-normal text-gray-400">(Updating...)</span>}
              </CardTitle>
            </CardHeader>
            <CardContent>
              {cart.length === 0 ? (
                ...
                <div className="space-y-6">
                  {cart.map((item) => (
                    ...
                    >
                      ...
                      {/* Controls Row (Qty, Price, Delete) */}
                      {/* On Mobile: This takes full width and spaces items out */}
                    ...
                    <div key={order.id} className="border border-gray-200 rounded-lg p-4 bg-white hover:shadow-md transition-shadow">
                      <div className="flex justify-between items-center mb-3 border-b border-gray-100 pb-2">
                        <div>
                          ...
                          <span className="text-xs text-gray-500 block ml-0.5">
                             {new Date(order.date).toLocaleDateString()}
                          </span>
                        </div>
                        <div className="text-right">
                          <span className="block font-bold text-green-700">₹{order.total}</span>
                          <span className="inline-block text-[10px] uppercase font-bold tracking-wider bg-yellow-100 text-yellow-800 px-2 py-0.5 rounded-sm">
                            {order.status}
                          </span>
                        </div>
                      </div>
